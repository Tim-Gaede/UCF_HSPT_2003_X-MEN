...
int ScoreWolverine(int y, int x)
{
  int score = 0;
  if (y > 0)
  {
    if (x > 0)
      score += (board[y-1][x-1] == 1);
    score += (board[y-1][x] == 1);
    if (x < width-1)
      score += (board[y-1][x+1] == 1);
  }
